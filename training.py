...
def train_model(model, criterion, dataloaders, optimizer, metrics: dict, results_path, num_epochs, metric_threshold: float = 0.1) -> torch.nn.Module:
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        with open(os.path.join(results_path, 'log.csv'), 'a', newline='') as csvfile:
            ...
            if phase == 'Test' and f1_score > best_f1_score:
                ...
                best_model_wts = copy.deepcopy(model.state_dict())
                torch.save(model, f"results/weights_epoch-{batchsummary['epoch']}.pth")
